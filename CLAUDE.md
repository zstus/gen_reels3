...
# Ken Burns 및 텍스트 처리 로그 (최신)
🔳 정사각형 크롭: 1920x1080 → 1080x1080  
🔳 최종 리사이즈: 1080x1080 → 716x716
🎬 Ken Burns 패턴: 2 (우→좌 패닝)
🎯 Cubic ease-in-out 적용: 더 부드러운 애니메이션
📍 텍스트 위치: bottom (하단 배치)
✏️ 30px 폰트 + 2px 외곽선 적용
...
**최근 업데이트**: 2024년 9월 3일  
**주요 개선**: 
- 🌐 **URL 자동 릴스 생성**: 웹페이지 URL → ChatGPT → 7단계 릴스 대본 자동 생성
- 🔧 **OpenAI SDK 호환성**: v1.50+ 버전 지원, proxies 에러 해결
- 🎨 **텍스트 크기 향상**: 30px → 36pt 폰트로 가독성 극대화  
- 🤖 **AI 최적화**: BeautifulSoup 지능형 스크래핑 + GPT-3.5-turbo 릴스 최적화
- 📦 **의존성 업데이트**: beautifulsoup4, lxml, python-dotenv 추가
